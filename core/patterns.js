...
// (SKILL.md — spec at https://agentskills.io/specification, fetched
// 2026-07-31: frontmatter `name` is 1-64 chars, lowercase alnum/hyphen only,
// no leading/trailing/consecutive hyphen, AND MUST MATCH THE PARENT
// DIRECTORY NAME; `description` is 1-1024 chars). Rather than deriving
// `name` from a user-supplied --out (which could be any string), --out is
// treated as the PARENT directory and the skill itself is always written to
// <out>/learned-patterns/ — this guarantees name-matches-directory-name
// unconditionally instead of only for the default path.
const DEFAULT_SKILL_NAME = "learned-patterns";
const SKILL_EXPORT_HEADER = "Generated by devteam patterns export; regenerate to update; do not hand-edit.";
const SKILL_DESCRIPTION_MAX = 1024;

function resolveSkillExportDir(cwd, outDir) {
  const parent = outDir
    ? (path.isAbsolute(outDir) ? outDir : path.join(cwd, outDir))
    : path.join(cwd, ".devteam");
  return path.join(parent, DEFAULT_SKILL_NAME);
}

function domainLabel(domain) {
  return String(domain || "correctness")
    .split(/[-_\s]+/)
    .filter(Boolean)
...
function truncateDescription(text, max = SKILL_DESCRIPTION_MAX) {
  return text.length > max ? `${text.slice(0, max - 1).trimEnd()}…` : text;
}

function skillDescription(promoted, domains) {
  const text = promoted.length > 0
    ? `Project-specific engineering patterns learned from this project's devteam pipeline runs (${promoted.length} promoted pattern(s) across ${domains.length} domain(s): ${domains.join(", ")}). Use when implementing, reviewing, or fixing code in this project to avoid repeating previously-identified issues and to follow established project conventions.`
    : "Project-specific engineering patterns learned from this project's devteam pipeline runs. No patterns are promoted yet — run `devteam patterns promote <candidate-id>` after collecting observations, then re-export.";
  return truncateDescription(text);
}

function renderPatternSkillMd({ name, promoted }) {
  const byDomain = new Map();
  for (const pattern of promoted) {
    const domain = pattern.domain || "correctness";
    if (!byDomain.has(domain)) byDomain.set(domain, []);
    byDomain.get(domain).push(pattern);
  }
  ...
  lines.push(`description: ${JSON.stringify(skillDescription(promoted, domains))}`);
  lines.push("---");
  lines.push("");
  lines.push(`<!-- ${SKILL_EXPORT_HEADER} -->`);
  lines.push("");
  lines.push("# Learned Project Patterns");
  lines.push("");
  lines.push("Patterns promoted via `devteam patterns promote` from prior pipeline runs of this project. Each entry is operator-reviewed prevention guidance derived from real gate blockers, warnings, and follow-ups — not raw agent output.");
  for (const domain of domains) {
    lines.push("");
    lines.push(`## ${domainLabel(domain)}`);
    const items = byDomain.get(domain).slice().sort((a, b) => a.id.localeCompare(b.id));
    for (const pattern of items) {
      const observations = (pattern.evidence && pattern.evidence.observations) || 0;
      const lastReinforced = pattern.evidence && pattern.evidence.last_reinforced;
      const rationale = [
        `tier: ${pattern.tier}`,
        `${observations} observation(s)`,
        lastReinforced ? `last reinforced ${lastReinforced}` : null,
      ].filter(Boolean).join(", ");
      lines.push("");
      lines.push(`- ${pattern.prompt_text}`);
      lines.push(`  - _Rationale: ${rationale}._`);
    }
  }
  lines.push("");
  return lines.join("\n");
...
// Re-scans the fully rendered file with the same secret-scan path promote()
// uses. Defense in depth: a promoted pattern's prompt_text is scanned once at
// promotion time, but promoted.json is hand-editable (e.g. a demote/re-promote
// round-trip, or direct operator edits), so export re-verifies before writing
// anything a host would load as instructions.
function exportSkill({ cwd, outDir }) {
  const dir = resolveSkillExportDir(cwd, outDir);
  const name = DEFAULT_SKILL_NAME;
  const promoted = loadPromoted(cwd)
    .filter((item) => item.status === "promoted" && item.prompt_text)
    .sort((a, b) => a.id.localeCompare(b.id));
  const rendered = renderPatternSkillMd({ name, promoted });
  const findings = scanContent(rendered);
  if (findings.length > 0) {
    throw new Error(`patterns export --skill: rendered output looks secret-shaped: ${findings.map((f) => f.name).join(", ")}`);
  }
  ...
  return {
    ...
    domains: [...new Set(promoted.map((item) => item.domain || "correctness"))].sort(),
  };
}
