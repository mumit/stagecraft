...
2. **The core never spawns a model.** It emits prompts and validates outputs; host adapters decide how the model consumes a prompt. Pipeline stages call `adapter.invoke()` through the orchestrator. One-off workflows such as rulings, accessibility remediation, reflection, and read-only `devteam chat` call the same adapter boundary; core modules must not parse `headlessCommand` or spawn a host CLI themselves. The conversational core assembles a deterministic read-only snapshot, while the adapter alone owns model/agent transport.
