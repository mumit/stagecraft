...
2. **The core never spawns a model.** It emits prompts and validates outputs; host adapters decide how the model consumes a prompt. Pipeline stages call `adapter.invoke()` through the orchestrator. One-off workflows such as rulings, accessibility remediation, and reflection call the same adapter boundary through `core/adapters/invoke-task.js`; core modules must not parse `headlessCommand` or spawn a host CLI themselves.
