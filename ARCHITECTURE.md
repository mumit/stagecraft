...
2. **The core never spawns a model.** It emits prompts and validates gate JSON. Hosts decide how the model consumes the prompt. This includes `devteam chat`: core assembles a deterministic read-only snapshot, then calls the selected adapter's `invoke()` boundary; the adapter alone owns model/agent transport.
