...
- **Auto-fixes machine-diagnosable failures.** On a `code-defect` it clears the failing gate, writes the blockers into `pipeline/context.md` as cross-stage context, and re-dispatches — bounded by `autonomy.max_retries` (default 2). Transient dispatch failures (no gate written) back off and retry; structural ones (clean exit, no output) halt. Retry events include `retry_reason` and `backoff_class`; immediate non-timeout no-gate failures use short backoff while timeouts keep the full delay.
...
- **Safety rails.** An exclusive `pipeline/run.lock`, resumable `run-state.json` (`--resume`), a materialized `pipeline/run-plan.json`, a pre-dispatch `--budget-usd` cap, a per-stage `--timeout-ms`, and a `--max-iterations` guard. The run plan records resolved track provenance, the ordered preflight stage-disposition snapshot, candidate role/host/model routes, and a SHA-256 execution fingerprint. Resumes reuse the original artifact only when stable track/config/routing controls still match; dynamic right-sizing and upstream-gate conditions remain runtime decisions.
