...
A **track** is a named subset of pipeline stages. It tells Stagecraft how much rigor a change requires. Start with three primary assurance choices: `loop` for an iteration that does not deploy, `quick` for a bounded change that does, and `full` for audited/high-stakes delivery. The other tracks are specialist profiles selected by change shape, not additional points on an assurance slider: `nano`, `config-only`, `dep-update`, `hotfix`, `review-only`, `review-pr`, and `refactor`.

`loop` is the day-to-day default: the lightest track that still produces a brief, a build, a QA pass, and a review gate. `quick` adds a delivery path and broader verification without formal design or adversarial review. `full` is the **audited** path — every stage, including formal design and adversarial review — chosen when stakes or compliance justify the ceremony. `devteam assess` can recommend a specialist profile when file shape or intent is more informative than size; its output always compares `loop`, `quick`, and `full` so the primary tradeoff remains visible.
...
`devteam assess` automates this decision: given a change description and a file list it returns a `recommendedTrack`, a `confidence` level (`high | medium | low`), and the reasons. Specialized signals choose `hotfix`, `dep-update`, `config-only`, `nano`, or `quick`; otherwise the day-to-day fallback is `loop`. Security-triggered code work cannot remain on `loop`, `nano`, or `quick` because none contains security-review, so it promotes to `full`. Running `devteam assess` (no flags) writes the result to `pipeline/track.json` so `devteam run` picks it up automatically. Use `devteam assess --confirm` to set `source:"human"` (operator-confirmed). See [Track record (`pipeline/track.json`)](#track-record-pipelinetrackjson), [`ADR-006`](adr/006-track-inference-under-autonomy.md), and [`ADR-018`](adr/018-materialized-run-plan-and-loop-default.md).

**You don't have to run `devteam assess` yourself first.** If `devteam run` starts with no `--track`, no `pipeline/track.json`, and no `pipeline.custom_stages`, it runs the same `assess()` heuristics inline — at any confidence level, not just "high" — prints the recommendation and reasons before dispatching anything, and writes `pipeline/track.json` itself (`source:"inferred"`) so the decision is still a file you can read, diff, or override with `--track`. Generic feature descriptions now resolve to `loop` rather than paying full ceremony by default. This requires a `--feature`/description to assess; a bare `devteam run` with nothing to go on still falls through to `pipeline.default_track`, which `devteam init` writes as `loop`. Before dispatch, the resolved decision is expanded into `pipeline/run-plan.json`, including stage dispositions and candidate role/host/model routes. See [ADR-016](adr/016-assess-by-default.md) and [ADR-018](adr/018-materialized-run-plan-and-loop-default.md).
...
> **Note on the config.yml default.** `devteam init` writes `pipeline.default_track: loop`, matching the inference default in
> [ADR-018](adr/018-materialized-run-plan-and-loop-default.md) and the guidance above. `full` runs red-team adversarial review and
> formal design on every change, which is wasteful when most attack surfaces don't apply, so it is a deliberate choice rather than
> the resting state — reserve it for changes where the audited trail is worth the ceremony. Lighter tracks stay stoplist-guarded, so
> a change touching auth, payments, crypto, or migrations refuses to run on `loop` and tells you to use `full`.
>
> One value deliberately did **not** change: a config file that names no `default_track` at all still resolves to `full`. Picking a
> default for a new project and silently reducing rigor for an existing one that never chose are different decisions.
