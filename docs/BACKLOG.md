...
| E13 | **Two-project dogfood reliability follow-through** | 5 | 3 | [Phase 42](../plans/phase-42-dogfood-reliability.md). ✅ 42.1–42.3 unreleased: resume preserves caps/track/scoped stoplist rulings; structured retries prove role/path compatibility; ADR-022 adds exact-file documentation ownership shared by build, QA, review, and retry. Next P1: project-layout-aware QA and logical-run evidence semantics. P2: checkout-local dogfood bootstrap isolation. Supported by the [2026-08-19 Phase 41 no-go review](../plans/phase-41-evidence-review-2026-08.md); does not activate learning. |
...
- **D5 maturation — continuous adaptive routing.** Today D5 proposes role-level swaps; the mature form re-routes the *next* run based on the prior run's outcomes automatically. **Evidence baseline (2026-06-14, `plans/adaptive-routing-evidence.md`):** zero real-run telemetry at review time. Phase 17 starts durable collection from real autonomous dispatches; it does not backfill old gates, and it counts `devteam run` dispatches only — a `devteam stage` dispatch writes a gate and a corpus row but no durable event, because a repeatable ad-hoc invocation is not an independent routing observation (see the scope section in `plans/phase-17-durable-evidence-instrumentation.md`). Gate stays shut pending ≥5 durable dispatches per (role, host) pair across ≥2 real user projects and cost telemetry. ADR-007 Tier 1 (liveness heartbeat + observe-only stall probe) implemented in Phase 11.1; ADR-008 (advisory sweep + `--fail-on-advisory`) implemented in Phase 11.2; ADR-007 Tier 2 remains evidence-gated.
