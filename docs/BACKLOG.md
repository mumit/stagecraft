...
| D9 | **Verification efficiency: concurrency and receipts** ([#315](https://github.com/telus-labs/stagecraft/issues/315)) | 4 | 4 | First slice in progress: run independent suites with bounded concurrency, stable result ordering, output caps, and `resource_group` exclusivity. Remaining depth: content-addressed verification receipts that cannot go stale across source/test/dependency/command/env changes. |
| D10 | **Safe track/workstream right-sizing** ([#316](https://github.com/telus-labs/stagecraft/issues/316)) | 4 | 4 | First deterministic-skip slice in progress: autonomous runs record typed `skip-stage` audit events for configured and conditional skips, including trigger inputs, and `pipeline.force_stages` gives operators an override. Remaining depth: run-plan preview, high-confidence auto-track selection, and active workstream derivation. |
