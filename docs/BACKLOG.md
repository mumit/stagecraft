...
| D10 | **Safe track/workstream right-sizing** ([#316](https://github.com/telus-labs/stagecraft/issues/316)) | 4 | 4 | First slices in progress: `devteam run` emits a `run-plan` preview before dispatch, autonomous runs record typed `skip-stage` audit events for configured and conditional skips, `pipeline.force_stages` gives operators an override, and [ADR-019](adr/019-isolated-build-workstreams.md) adds opt-in Git-worktree isolation with deterministic reconciliation for parallel builders. Remaining depth: high-confidence auto-track selection and active workstream derivation. |
