...
| D10 | **Safe track/workstream right-sizing** ([#316](https://github.com/telus-labs/stagecraft/issues/316)) | 4 | 4 | ✅ Unreleased · `devteam run` emits a right-sized `run-plan`, auto-selects lighter tracks only at high confidence, seeds candidate active roles for Stage 01 confirmation, records deterministic skip-stage events with trigger inputs, and supports `pipeline.force_stages` / `pipeline.right_sizing: false` overrides. |
