...
| D10 | **Safe track/workstream right-sizing** ([#316](https://github.com/telus-labs/stagecraft/issues/316)) | 4 | 4 | First slice in progress: `devteam run` emits a `run-plan` preview with effective track/custom plan, included/total stages, configured skips, conditional-stage count, and base workstream count before dispatch. Remaining depth: deterministic active-role/stage derivation and auditable safe skips. |
