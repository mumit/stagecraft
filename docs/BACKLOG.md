...
| D10 | **Safe track/workstream right-sizing** ([#316](https://github.com/telus-labs/stagecraft/issues/316)) | 4 | 4 | First slices in progress: `devteam run` emits a `run-plan` preview before dispatch, autonomous runs record typed `skip-stage` audit events for configured and conditional skips, and `pipeline.force_stages` gives operators an override. Remaining depth: high-confidence auto-track selection and active workstream derivation. |
| D11 | **Per-host workstream scheduling and retry backoff** ([#317](https://github.com/telus-labs/stagecraft/issues/317)) | 4 | 3 | ✅ Unreleased · `routing.host_concurrency` caps already-ready workstreams per host, run logs record queue wait, critical-path reports surface queue time, and transient retry events include reason/backoff class. Stage-level DAG waves remain deferred behind ADR-defined dependency metadata. |
| D12 | **Pattern learning for agent growth** ([#332](https://github.com/telus-labs/stagecraft/issues/332)) | 5 | 4 | Proposed in [Phase 27](../plans/phase-27-pattern-learning.md). Add `devteam patterns` so reviewed blockers, warnings, positive examples, and auto-retry outcomes become project-local, privacy-bounded prompt guidance. Distinct from H3: advisory prevention before coding, not deterministic recipe creation. |
