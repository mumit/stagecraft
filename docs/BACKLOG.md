...
| D8 | **Critical-path telemetry and performance report** ([#312](https://github.com/telus-labs/stagecraft/issues/312), [#313](https://github.com/telus-labs/stagecraft/issues/313)) | 5 | 3 | ✅ First slice in progress: `devteam performance critical-path` reports dispatch wall time, merge time, retry delay, workstream compute, parallel savings, telemetry coverage, and repeated verification-command candidates from run logs and stamped gates. Remaining depth: per-stamp substeps and larger real-run p50/p95 baseline collection before scheduling changes. |
