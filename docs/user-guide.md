...
- **Did every gate genuinely pass, or did anything get rubber-stamped?** Read each `pipeline/gates/stage-NN.json` and the corresponding artifact. A PASS with a sparse `## Verify` section in `pipeline/pr-*.md` is a yellow flag. Stage 4 (build, per-role during dispatch + a merged full-suite check after merge — Phase 31.1), Stage 4a, and Stage 6 are orchestrator-stamped — look for `_orchestrator_stamped` on the workstream and merged gates. Run `devteam verify stage-04a` and `devteam verify stage-06` to re-stamp those two on demand (stage-04 re-stamping is only automatic on `devteam run`, not yet available as an on-demand `devteam verify stage-04`); if the current code still passes, the recorded PASS was real.
