...
- **`--budget-tokens` counts cache reads and writes.** On an agentic host they
  are almost the whole consumption: a measured `loop` run reported 52 uncached
  input and 11,293 output against 1,363,880 cache tokens, so a total of in+out
  alone was 121x low and a cap could not bind. The run summary reports
  `tokens_used` (the total), with `tokens_in` / `tokens_out` / `tokens_cached`
  separable — cache reads bill well below uncached input, so a large total is
  not a proportionally large bill, and `--budget-usd` remains the control to
  budget money against.

- **Cached input tokens** are provider-dependent. Codex, openai-compat, and Claude Code all record `cached_tokens` when reported; Claude Code additionally reports `cache_creation_tokens`, so `devteam performance` can show reads-per-write — the ratio that says whether the phase-32.1 byte-stable prefix is actually being reused or just re-created each dispatch. Cached tokens remain a subset of input and are never added again to token-budget totals. Local openai-compat cost calculation still prices full input at the uncached rate; Claude Code's observed dollar cost comes from the CLI and already reflects provider billing.
