...
| `retry-ownership` | A structured retry named target paths, but none of the candidate build roles can write every target under the existing `roleWrites` contract. Gates remain intact and no host was invoked. | Read `retry_ownership.{target_paths,candidate_roles}`. Fix manually, correct `stage-02.file_ownership`, or add a narrowly scoped owner through the stage contract before resuming. Do not broaden an unrelated role just to make the retry pass. |
