...
Every gate should carry a `"track"` field. Valid values: `full`, `quick`, `nano`, `config-only`, `dep-update`, `hotfix`, `loop`. The validator emits an advisory (non-blocking) when the field is missing or carries an unrecognised value. Treat missing as `full` for compatibility.
