...
// ADR-023 removed the C2 fallback chain. It existed only to squeeze a
// /goal-prefixed prompt under claude-code's 4,000-char slash-command limit,
// and it did so by dropping patchItems and the inlined framework from build
// and qa — the two stages that needed them most. Nothing composes /goal any
// more, no host declares promptCharLimit, and prompts are piped to stdin, so
// there is no ceiling left to fall back from. The replacement contract is
// asserted in tests/goal-loop.test.js.
