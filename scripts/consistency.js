...
// from bin/devteam's global surface and the per-command flag schemas in
// core/cli/commands/. This check verifies that the committed file equals what
// the generator would produce now, catching both manual edits and surface drift.
//
// Only runs in full-repo mode.
...
// evolves. This check reads the explicit Freshness line's "captured at" or
// "verified at" vX.Y stamp and warns when it is more than one minor behind
// package.json (D6.1). Scoping the match to the label avoids mistaking an older
// transcript's provenance for the current document stamp.
//
...
function checkExampleMdFreshnessStamp() {
  ...
  const stampMatch = content.match(/\*\*Freshness:\*\*\s+(?:captured|verified) at v(\d+)\.(\d+)/i);
  if (!stampMatch) {
    advisory("example-freshness",
      "EXAMPLE.md has no freshness stamp — add \"**Freshness:** verified at vX.Y\" near the top (D6.1)");
    return;
  }
